...
namespace VMAP
{
	...
	WmoLiquid::WmoLiquid(g3d_uint32 width,G3D::g3d_uint32 height, const Vector3 &corner,G3D::g3d_uint32 type):
	...
	bool WmoLiquid::GetLiquidHeight(const Vector3 &pos, float &liqHeight) const
	{
		...
		g3d_uint32 tx =G3D::g3d_uint32(tx_f);
		if (tx<0 || tx >= iTilesX) return false;
		float ty_f = (pos.y - iCorner.y)/LIQUID_TILE_SIZE;
		g3d_uint32 ty =G3D::g3d_uint32(ty_f);
	...
	g3d_uint32 WmoLiquid::GetFileSize()
	{
		return 2 * sizeof(g3d_uint32) +
				sizeof(Vector3) +
	...
	{
		bool result = true;
		if (result && fwrite(&iTilesX, sizeof(g3d_uint32), 1, wf) != 1) result = false;
		if (result && fwrite(&iTilesY, sizeof(g3d_uint32), 1, wf) != 1) result = false;
		if (result && fwrite(&iCorner, sizeof(Vector3), 1, wf) != 1) result = false;
		if (result && fwrite(&iType, sizeof(g3d_uint32), 1, wf) != 1) result = false;
		g3d_uint32 size = (iTilesX + 1)*(iTilesY + 1);
		if (result && fwrite(iHeight, sizeof(float), size, wf) != size) result = false;
		size = iTilesX*iTilesY;
		if (result && fwrite(iFlags, sizeof(g3d_uint8), size, wf) != size) result = false;
		return result;
	}
	...
	bool WmoLiquid::readFromFile(FILE *rf, WmoLiquid *&out)
	{
		...
		if (result && fread(&liquid->iTilesX, sizeof(g3d_uint32), 1, rf) != 1) result = false;
		if (result && fread(&liquid->iTilesY, sizeof(g3d_uint32), 1, rf) != 1) result = false;
		if (result && fread(&liquid->iCorner, sizeof(Vector3), 1, rf) != 1) result = false;
		if (result && fread(&liquid->iType, sizeof(g3d_uint32), 1, rf) != 1) result = false;
		g3d_uint32 size = (liquid->iTilesX + 1)*(liquid->iTilesY + 1);
		liquid->iHeight = new float[size];
		if (result && fread(liquid->iHeight, sizeof(float), size, rf) != size) result = false;
		size = liquid->iTilesX * liquid->iTilesY;
		liquid->iFlags = new G3D::g3d_uint8[size];
		if (result && fread(liquid->iFlags, sizeof(g3d_uint8), size, rf) != size) result = false;
		if (!result)
			delete liquid;
		out = liquid;
		return result;
	...
	{
		bool result = true;
		g3d_uint32 chunkSize, count;

		if (result && fwrite(&iBound, sizeof(G3D::AABox), 1, wf) != 1) result = false;
		if (result && fwrite(&iMogpFlags, sizeof(g3d_uint32), 1, wf) != 1) result = false;
		if (result && fwrite(&iGroupWMOID, sizeof(g3d_uint32), 1, wf) != 1) result = false;

		// write vertices
		if (result && fwrite("VERT", 1, 4, wf) != 4) result = false;
		count = (g3d_uint32)vertices.size();
		chunkSize = sizeof(g3d_uint32)+ sizeof(Vector3)*count;
		if (result && fwrite(&chunkSize, sizeof(g3d_uint32), 1, wf) != 1) result = false;
		if (result && fwrite(&count, sizeof(g3d_uint32), 1, wf) != 1) result = false;
		...
		count = (g3d_uint32)triangles.size();
		chunkSize = sizeof(g3d_uint32)+ sizeof(MeshTriangle)*count;
		if (result && fwrite(&chunkSize, sizeof(g3d_uint32), 1, wf) != 1) result = false;
		if (result && fwrite(&count, sizeof(g3d_uint32), 1, wf) != 1) result = false;
	...
	bool GroupModel::readFromFile(FILE *rf)
	{
		...
		if (result && fread(&iMogpFlags, sizeof(g3d_uint32), 1, rf) != 1) result = false;
		if (result && fread(&iGroupWMOID, sizeof(g3d_uint32), 1, rf) != 1) result = false;

		// read vertices
		if (result && !readChunk(rf, chunk, "VERT", 4)) result = false;
		if (result && fread(&chunkSize, sizeof(g3d_uint32), 1, rf) != 1) result = false;
		if (result && fread(&count, sizeof(g3d_uint32), 1, rf) != 1) result = false;
		if (!count) // models without (collision) geometry end here, unsure if they are useful
			return result;
	...
	g3d_uint32 GroupModel::GetLiquidType() const
	{
	...
	bool WorldModel::writeFile(const std::string &filename)
	{
		...
		chunkSize = sizeof(g3d_uint32) + sizeof(g3d_uint32);
		if (result && fwrite(&chunkSize, sizeof(g3d_uint32), 1, wf) != 1) result = false;
		if (result && fwrite(&RootWMOID, sizeof(g3d_uint32), 1, wf) != 1) result = false;

		// write group models
		count = (g3d_uint32)groupModels.size();
		if(count)
		{
			if (result && fwrite("GMOD", 1, 4, wf) != 4) result = false;
			//chunkSize = sizeof(g3d_uint32)+ sizeof(GroupModel)*count;
			//if (result && fwrite(&chunkSize, sizeof(g3d_uint32), 1, wf) != 1) result = false;
			if (result && fwrite(&count, sizeof(g3d_uint32), 1, wf) != 1) result = false;
			for (g3d_uint32 i=0; i<groupModels.size() && result; ++i)
	...
	bool WorldModel::readFile(const std::string &filename)
	{
		...
		if (result && fread(&chunkSize, sizeof(g3d_uint32), 1, rf) != 1) result = false;
		if (result && fread(&RootWMOID, sizeof(g3d_uint32), 1, rf) != 1) result = false;

		// read group models
		if (result && readChunk(rf, chunk, "GMOD", 4))
		{
			//if (fread(&chunkSize, sizeof(g3d_uint32), 1, rf) != 1) result = false;

			if (result && fread(&count, sizeof(g3d_uint32), 1, rf) != 1) result = false;
			if (result) groupModels.resize(count);
			//if (result && fread(&groupModels[0], sizeof(GroupModel), count, rf) != count) result = false;
			for (g3d_uint32 i=0; i<count && result; ++i)
