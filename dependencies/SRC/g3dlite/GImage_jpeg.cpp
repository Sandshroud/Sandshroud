...
void GImage::decodeJPEG(
	...
	jpeg_memory_src(&cinfo, const_cast<g3d_uint8*>(input.getCArray()), input.size());
	...
	while (cinfo.output_scanline < cinfo.output_height) {
	    ...
	    case 1:
            ...
            {
                g3d_uint8* scan     = &(m_byte[loc * 3]);
                g3d_uint8* endScan  = scan + (m_width * 3);
                g3d_uint8* t        = *temp;
