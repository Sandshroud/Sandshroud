...
// Byte conversion is required here
#ifdef USING_BIG_ENDIAN
template<size_t T> inline void convertBytes(char *val)
{
    std::swap(*val, *(val + T - 1));
    convertBytes<T - 2>(val + 1);
}
// C1202 fix
template<> inline void convertBytes<0>(char *) {}
template<> inline void convertBytes<1>(char *) {}
template<typename T> void EndianConvert(T*val) { convertBytes<sizeof(T)>((char *)val); }
#else
