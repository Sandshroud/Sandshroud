...
bool Spell::GenerateTargets(SpellCastTargets *t)
{
	...
	for(uint32 i = 0; i < 3; ++i)
	{
		...
		if(TargetType & SPELL_TARGET_AREA_RANDOM)
		{
			...
			while(sWorld.Collision && !CollideInterface.CheckLOS(m_caster->GetMapId(), m_caster->GetInstanceID(), m_caster->GetPhaseMask(), m_caster->GetPositionX(), m_caster->GetPositionY(), m_caster->GetPositionZ(), t->m_destX, t->m_destY, t->m_destZ));
			result = true;
		}
...
uint8 Spell::CanCast(bool tolerate)
{
	...
	if( p_caster != NULL )
	{
		...
		if( GetSpellProto()->Id == 7266 )
		{
			...
			{
				if(CollideInterface.IsIncity(p_caster->GetMapId(), p_caster->GetPositionX(), p_caster->GetPositionY(), p_caster->GetPositionZ()))
					return SPELL_FAILED_NO_DUELING;
			}
			else
			{
				AreaTable* at = dbcArea.LookupEntry( p_caster->GetPAreaID() );
				if(at == NULL)
					at = dbcArea.LookupEntry( p_caster->GetAreaID() );
				if(at == NULL)
		...
		if(target != NULL)
		{
			...
			if( p_caster != NULL )
			{
				...
				if(target->IsPlayer())
				{
					...
					{
						AreaTable* atCaster = dbcArea.LookupEntry( p_caster->GetPAreaID() );
						AreaTable* atTarget = dbcArea.LookupEntry( TO_PLAYER( target )->GetPAreaID() );
