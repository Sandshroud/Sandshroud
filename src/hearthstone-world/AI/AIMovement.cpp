...
void AI_Movement::Update(uint32 p_time)
{
    ...
    if(getAIState() == STATE_WANDER && getCreatureState() == STOPPED)
    {
        ...
        {
            if( !CollideInterface.GetFirstPoint( m_Unit->GetMapId(), m_Unit->GetInstanceID(), m_Unit->GetPhaseMask(), m_Unit->GetPositionX(), m_Unit->GetPositionY(), m_Unit->GetPositionZ(),
                Fx, Fy, m_Unit->GetPositionZ() + 1.5f, Fx, Fy, Fz, -3.5f ) )
            {
                // clear path?
                Fz = CollideInterface.GetHeight( m_Unit->GetMapId(), m_Unit->GetInstanceID(), m_Unit->GetPhaseMask(), Fx, Fy, m_Unit->GetPositionZ() );
                if( Fz == NO_WMO_HEIGHT )
            ...
            else
            {
                ...
                float fz2 = CollideInterface.GetHeight(m_Unit->GetMapId(), m_Unit->GetInstanceID(), m_Unit->GetPhaseMask(), Fx, Fy, Fz);
