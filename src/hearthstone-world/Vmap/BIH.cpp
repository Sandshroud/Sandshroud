...
{
	uint32 treeSize = (uint32)tree.size();
	uint32 check=0, count=0;
	check += (uint32)fwrite(&bounds.low(), sizeof(float), 3, wf);
	check += (uint32)fwrite(&bounds.high(), sizeof(float), 3, wf);
	check += (uint32)fwrite(&treeSize, sizeof(uint32), 1, wf);
	check += (uint32)fwrite(&tree[0], sizeof(uint32), treeSize, wf);
	count = (uint32)objects.size();
	check += (uint32)fwrite(&count, sizeof(uint32), 1, wf);
	check += (uint32)fwrite(&objects[0], sizeof(uint32), count, wf);
...
bool BIH::readFromFile(FILE *rf)
{
	...
	Vector3 lo, hi;
	uint32 check=0, count=0;
	check += (uint32)fread(&lo, sizeof(float), 3, rf);
	check += (uint32)fread(&hi, sizeof(float), 3, rf);
	bounds = AABox(lo, hi);
	check += (uint32)fread(&treeSize, sizeof(uint32), 1, rf);
	tree.resize(treeSize);
	check += (uint32)fread(&tree[0], sizeof(uint32), treeSize, rf);
	check += (uint32)fread(&count, sizeof(uint32), 1, rf);
	objects.resize(count); // = new uint32[nObjects];
	check += (uint32)fread(&objects[0], sizeof(uint32), count, rf);
	return check == (3 + 3 + 2 + treeSize + count);
}
