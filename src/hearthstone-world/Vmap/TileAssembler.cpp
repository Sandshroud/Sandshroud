...
namespace VMAP
{
		...
		for (MapData::iterator map_iter = mapData.begin(); map_iter != mapData.end() && success; ++map_iter)
		{
			...
			for (tile = tileEntries.begin(); tile != tileEntries.end(); ++tile)
			{
				...
				if (spawn.flags & MOD_WORLDSPAWN) // WDT spawn, saved as tile 65/65 currently...
					continue;
				uint32 nSpawns = (uint32)tileEntries.count(tile->first);
	...
	bool TileAssembler::readMapSpawns()
	{
		...
		while (!feof(dirf))
		{
			...
			check += (uint32)fread(&mapID, sizeof(uint32), 1, dirf);
			if (check == 0) // EoF...
				break;
			check += (uint32)fread(&tileX, sizeof(uint32), 1, dirf);
			check += (uint32)fread(&tileY, sizeof(uint32), 1, dirf);
