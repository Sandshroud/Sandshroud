...
void WorldSession::HandleItemQuerySingleOpcode( WorldPacket & recv_data )
{
    ...
    if(!itemProto)
    {
        sLog.Debug( "WORLD"," Unknown item id 0x%.8X", itemid );
        return;
    }
...
void WorldSession::HandleBuyItemInSlotOpcode( WorldPacket & recv_data ) // drag & drop
{
    ...
    sLog.Debug( "WORLD"," Received CMSG_BUY_ITEM_IN_SLOT" );
    ...
    if( it->MaxCount > 0 && ci.amount*amount > (uint32)it->MaxCount)
    {
    ...
    if(slot != INVENTORY_SLOT_NOT_SET)
    {
        ...
        else
        {
            ...
            if(bagslot == INVENTORY_SLOT_NOT_SET || (c->GetProto() && (int32)slot > c->GetProto()->ContainerSlots))
            {
                _player->GetItemInterface()->BuildInventoryChangeError(NULLITEM, NULLITEM, INV_ERR_ITEM_DOESNT_GO_TO_SLOT);
                return;
            }
    ...
    if(oldItem != NULL)
    {
        ...
        if(it->MaxCount > 0 && (oldItem->GetUInt32Value(ITEM_FIELD_STACK_COUNT) + count_per_stack) > (uint32)it->MaxCount)
        {
//          sLog.outDebug( "SUPADBG can't carry #2" );
    ...
    SendPacket( &data );
    sLog.Debug( "WORLD"," Sent SMSG_BUY_ITEM" );
