...
public:
    ...
    HEARTHSTONE_INLINE std::map<uint32, CreatureItem>::iterator GetSellItemBegin() { return m_SellItems->begin(); }
    HEARTHSTONE_INLINE std::map<uint32, CreatureItem>::iterator GetSellItemEnd()   { return m_SellItems->end(); }
    HEARTHSTONE_INLINE void RemoveSellItem(std::map<uint32, CreatureItem>::iterator itr) { m_SellItems->erase(itr); }
