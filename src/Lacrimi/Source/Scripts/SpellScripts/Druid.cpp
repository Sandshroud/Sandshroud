...
bool Starfall(uint32 i, Spell* pSpell)
{
    ...
    {
        SpellEntry* spellInfo = dbcSpell.LookupEntry( pSpell->damage );
        if(spellInfo != NULL)
        {
...
void FerociousBite(uint32 i, Spell* pSpell, uint32 effect)
{
    if( effect == SPELL_EFFECT_SCHOOL_DAMAGE )
    {
        if( pSpell->p_caster != NULL )
        {
            ...
            pSpell->damage += float2int32( ( pSpell->p_caster->m_comboPoints * pSpell->p_caster->GetAP() * 0.07f ) + ( (usedpower * pSpell->GetSpellProto()->EffectDamageMultiplier[1] + pSpell->p_caster->GetAP()) / 410.0f ) );
...
void CatSwipe(uint32 i, Spell* pSpell, uint32 effect)
{
    if( effect == SPELL_EFFECT_SCHOOL_DAMAGE )
    {
        if( pSpell->p_caster != NULL )
        {
            ...
            float wpdmg;
            if(it)
                wpdmg = (it->GetProto()->minDamage + it->GetProto()->maxDamage) * 0.2f;
            else
