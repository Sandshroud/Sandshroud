...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//Class TargetType
TargetType::TargetType( uint32 pTargetGen, TargetFilters pTargetFilter, uint32 pMinTargetNumber, uint32 pMaxTargetNumber )
{
...
{
    return _unit->GetAIInterface()->getCanMove();
}
...
void MoonScriptCreatureAI::SetBehavior(BehaviorType pBehavior)
{
    ...
    _unit->GetAIInterface()->SetBehaviorType(pBehavior);
}

BehaviorType MoonScriptCreatureAI::GetBehavior()
{
    return _unit->GetAIInterface()->GetBehaviorType();
}
...
{
    _unit->m_AuraInterface.RemoveAllAuras();
}
...
void MoonScriptCreatureAI::AddLootFromTable(Unit *pTarget, LootTable* pTable, uint32 pCount)
{
    ...
    for (uint32 count = 0; count < pCount; ++count)
    {
        ...
        for (LootTable::iterator it = pTable->begin(); it != pTable->end(); ++it)
        {
            ...
            if (result <= sum)
            {
                lootmgr.AddLoot(pTarget->GetLoot(), (*it).mItemID, (*it).mMinCount, (*it).mMaxCount, (*it).mFFA);
                break;
            }
        }
    }
...
{
    lootmgr.AddLoot(pTarget->GetLoot(), pItemID, pMinCount, pMaxCount, pFFA);
}
...
void MoonScriptCreatureAI::AddRareLoot(Unit *pTarget, uint32 pItemID, float pPercentChance)
{
    ...
    if (result <= pPercentChance)
        lootmgr.AddLoot(pTarget->GetLoot(), pItemID, 1, 1, 0);
}
...
void MoonScriptCreatureAI::AIUpdate(MapManagerScript* MMScript, uint32 p_time)
{
    SpellDesc*  Spell;
    uint32      CurrentTime = (uint32)time(NULL);
...
Unit *MoonScriptCreatureAI::ChooseBestTargetInArray( UnitArray& pTargetArray, TargetFilters pTargetFilter )
{
...
bool MoonScriptCreatureAI::IsValidUnitTarget( Object *pObject, TargetFilters pFilter, float pMinRange, float pMaxRange )
{
    ...
    if ( pFilter != TargetFilter_None )
    {
        ...
        if ( ( ~pFilter & TargetFilter_Corpse ) && ( pFilter & TargetFilter_Friendly ) ) 
        {
            ...
            if ( FactionSystem::isHostile( _unit, UnitTarget ) || _unit->GetAIInterface()->getThreatByPtr( UnitTarget ) > 0 )
                return false;
        };
    };
